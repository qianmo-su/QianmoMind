...
def build_arg_parser():
    ...
    parser.add_argument("--checkpoint", type=str, default="checkpoints/sft_identity_light/last.pt")
...
def load_model(checkpoint_path, tokenizer, device):
    ...
    if config_dict is None:
        raise ValueError("Checkpoint has no config. Please use a checkpoint saved by trainer scripts.")
...
def run_generation(args, model, tokenizer, device, prompt):
...
def main():
    ...
    if args.prompt:
        print(run_generation(args, model, tokenizer, device, args.prompt))
        return
    ...
    while True:
        ...
        if not prompt:
            continue
        print(f"assistant> {run_generation(args, model, tokenizer, device, prompt)}")
